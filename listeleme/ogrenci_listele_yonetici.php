...
<form method="GET" class="  col-sm-12 col-md-6 col-lg-6 col-xl-6">
  <div class="form-group">
    <label for="danisman_id">Danışman Seçin:</label>
    <select class="form-control" name="danisman_id" id="danisman_id" onchange="this.form.submit()">
      <option value="">Tüm Danışmanlar</option>
      <option value="x" <?php if ($danisman_id == 'x') { echo 'selected'; } ?>>Danışman Atanmamış Öğrenciler</option>
      <?php
      while ($row_danisman = $result_danisman->fetch_assoc()) {
        $selected = "";
        if ($danisman_id == $row_danisman['kullanici_id']) {
          $selected = "selected";
        }
        echo "<option value='" . htmlspecialchars($row_danisman['kullanici_id']) . "' $selected>" . htmlspecialchars($row_danisman['kullanici_isim']) . " " . htmlspecialchars($row_danisman['kullanici_soyisim']) . "</option>";
      }
      ?>
    </select>
  </div>
...
<table class="table table-striped" id="ogrenci_tablosu">
  ...
  <tbody>
    <?php
    while ($row_ogrenci = $result_ogrenci->fetch_assoc()) {
      ...
      // Bulunan aşamayı seçin ve ismini ekrana yazdırın
      if ($bulunan_asama > 0) {
      ...
      // Öğrenci satırını oluşturun ve tez başvuru sayısını görüntüleyin
      echo "<tr>";
      ...
      echo "<td><a class='btn btn-success btn-sm' href='ogrenci_detay.php?id=" . $ogrenci_id . "'>Detay Görüntüle</a></td>";
      echo "<td><a class='btn btn-primary btn-sm' href='ogrenci_duzenle.php?id=" . $ogrenci_id . "'>Düzenle</a></td>";
      echo "<td><a class='btn btn-danger btn-sm' href='islemyonetici/ogrenci_sil.php?id=" . $ogrenci_id . "' onclick='return confirm(\"Emin misiniz?\")'>Sil</a></td>";
      echo "</tr>";
    }
    ?>
  </tbody>
